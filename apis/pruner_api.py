...
MODEL_NAME_OR_PATH = os.environ.get(
    PRUNER_MODEL_NAME_ENV, PRUNER_MODEL_NAME
...
class PruneRequest(BaseModel):
    ...
    top_k: int = Field(
        5,
        description="specifica il numero di passaggi meglio classificati da mantenere per ogni domanda.",
    )
    threshold: float = Field(
        0.05, description="quale soglia utilizzare per il pruning del contesto."
    )
...
class PruneResponse(BaseModel):
    ...
    pruned_docs: List[str] = Field(
        ..., description="Lista dei pruned (tagliare + rerank) documenti"
    )
