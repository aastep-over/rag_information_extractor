...
from utils.eval_accuracy import (
    accuracy_overall,
    accuracy_per_company,
    percentage_of_fields_extracted,
)
...
def return_single_run_summary(
    ...
    percent_field_extracted = percentage_of_fields_extracted(
        combined_pred_data, combined_raw_data
    )
...
def write_summary(
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        ...
        if USE_PARENT_CHUNKS:
            ...
            w(
                "\t\tAvg CP Overall:",
                context_pr_output_parent["overall"].get("Precision", 0.0),
            )
            w("\t\tAvg CP Per Group:")
            for group, data in context_pr_output_parent["per_group"][
                "Precision"
            ].items():
                w(f"\t\t\t{group}: {data}")
        ...
        w(
            "\t\tAvg CP Overall:",
            context_pr_output_children["overall"].get("Precision", 0.0),
        )
        w("\t\tAvg CP Per Group:")
        ...
        if USE_PARENT_CHUNKS:
            ...
            w(
                "\t\tAvg CR Overall:",
                context_pr_output_parent["overall"].get("Recall", 0.0),
            )
            w("\t\tAvg CR Per Group:")
        ...
        w(
            "\t\tAvg CR Overall:",
            context_pr_output_children["overall"].get("Recall", 0.0),
        )
        w("\t\tAvg CR Per Group:")
        ...
        if USE_PARENT_CHUNKS:
            ...
            for group, data in context_pr_output_parent["per_subgroup"][
                "Precision"
            ].items():
                w(f"\t\t{group}:")
        ...
        w("\tAvg CP Per Sub-Group:")
        for group, data in context_pr_output_children["per_subgroup"][
            "Precision"
        ].items():
            w(f"\t\t{group}:")
            for sg, sg_data in data.items():
        ...
        w("Overall Context Recall:\n")
        w("\tAvg CR Overall:", context_pr_output_children["overall"].get("Recall", 0.0))
        w("\tAvg CR Per Group:")
        for group, data in context_pr_output_children["per_group"]["Recall"].items():
            w(f"\t\t{group}: {data}")
...
if __name__ == "__main__":
    ...
    run_names = [
        f
        for f in os.listdir(Path(BASE_DIR, args.runs_dir))
        if os.path.isdir(Path(BASE_DIR, args.runs_dir, f))
    ]
    print(args.runs_dir)
    ...
    combined_pred_jsons = [
        Path(BASE_DIR, args.runs_dir, run, "pred.json") for run in run_names
    ]
    match_scores_jsons = [
        Path(BASE_DIR, args.runs_dir, run, "match_scores.json") for run in run_names
    ]
    match_scores_qa_jsons = [
        Path(BASE_DIR, args.runs_dir, run, "match_scores_qa.json") for run in run_names
    ]

    assert combined_raw_json.exists(), f"File not found: {combined_raw_json}"
    for i in range(len(combined_pred_jsons)):
        assert combined_pred_jsons[
            i
        ].exists(), f"File not found: {combined_pred_jsons[i]}"
        assert match_scores_jsons[
            i
        ].exists(), f"File not found: {match_scores_jsons[i]}"
        assert match_scores_qa_jsons[
            i
        ].exists(), f"File not found: {match_scores_qa_jsons[i]}"
    ...
    for i in range(len(combined_pred_jsons)):
        (
            ...
            combined_raw_json,
            combined_pred_jsons[i],
            match_scores_jsons[i],
            match_scores_qa_jsons[i],
        )
    ...
    percent_field_extracted = sum(percent_field_extracted_all_runs) / len(
        percent_field_extracted_all_runs
    )
