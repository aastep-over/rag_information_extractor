...
def accuracy_per_company(match_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "overall": {
            "correct": c_all,
            "total": t_all,
            "accuracy": (c_all / t_all) if t_all else 0.0,
        },
        "per_group": per_group,
    }
...
def percentage_of_fields_extracted(
    combined_pred_data: Dict[str, Any], combined_raw_data: Dict[str, Any]
) -> float:
        ...
        for group_name in combined_raw_data[company].keys():
            ...
            for sg_name in raw_data.keys():
                for k in raw_data[sg_name].keys():
                    if raw_data[sg_name][k]:
                        non_empty_raw_fields += 1
                    if pred_data[sg_name][k] and raw_data[sg_name][k]:
                        extracted_fields += 1
