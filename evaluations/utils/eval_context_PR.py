...
from ragas.metrics import IDBasedContextPrecision  # ! pip install rapidfuzz
...
def context_precision_ragas(
    ...
    if id_based:
        ...
        retrieved_contexts_str: List[str] = [
            " ".join(chunks[i].page_content.split()) for i in retrieved_contexts
        ]
        re_ranked_contexts_str: List[str] = [
            " ".join(chunks[i].page_content.split()) for i in re_ranked_contexts
        ]
...
def context_PR_per_company(
    ...
    total_count_cp = {
        "retrieved_CP": 0,
        "re_ranked_CP": 0,
    }  # precision should always count, but keep symmetric
    ...
    for group_name, group in results_qa.items():
        ...
        for sg_name, sg in group.items():
            ...
            if use_parent_chunks:
                ...
                retrieved_docs = results_contexts[group_name]["retrieved_docs"][
                    sg_name
                ].get("parents", [])
                re_ranked_docs = results_contexts[group_name]["re_ranked_docs"][
                    sg_name
                ].get("parents", [])

                ref_answer = ref_qa[group_name][sg_name]["A"]
                reference_context = ref_contexts[group_name][sg_name]
                ref_context_ids = ref_contexts_ids[group_name][sg_name].get(
                    "parents", []
                )
        ...
        avg_group_score_cp = {
            k: (
                round(total_group_score_cp[k] / count_group_cp[k], 3)
                if count_group_cp[k] > 0
                else None
            )
        ...
        avg_group_score_cr = {
            k: (
                round(total_group_score_cr[k] / count_group_cr[k], 3)
                if count_group_cr[k] > 0
                else None
            )
    ...
    avg_score_cp = {
        k: (
            round(total_score_cp[k] / total_count_cp[k], 3)
            if total_count_cp[k] > 0
            else None
        )
    ...
    avg_score_cr = {
        k: (
            round(total_score_cr[k] / total_count_cr[k], 3)
            if total_count_cr[k] > 0
            else None
        )
        for k in total_score_cr
    }

    return {"Precision": avg_score_cp, "Recall": avg_score_cr, "per_group": per_group}
...
def context_PR_overall(
    ...
    for res in per_company.values():
        for g, stats in res["per_group"].items():
            ...
            # group-level CP/CR
            _add_to_sums_and_counts(
                group_cp_sums[g], group_cp_counts[g], stats["Precision"]
            )
            _add_to_sums_and_counts(
                group_cr_sums[g], group_cr_counts[g], stats["Recall"]
            )
            ...
            for sg_name, sg_stats in stats["per_subgroup"].items():
                ...
                _add_to_sums_and_counts(
                    subgroup_cp_sums[g][sg_name],
                    subgroup_cp_counts[g][sg_name],
                    sg_stats["Precision"],
                )
                _add_to_sums_and_counts(
                    subgroup_cr_sums[g][sg_name],
                    subgroup_cr_counts[g][sg_name],
                    sg_stats["Recall"],
                )

    # averages per-group
    group_avg_score_cp = {
        g: _avg_from_sums_counts(group_cp_sums[g], group_cp_counts[g])
        for g in group_cp_sums
    }
    group_avg_score_cr = {
        g: _avg_from_sums_counts(group_cr_sums[g], group_cr_counts[g])
        for g in group_cr_sums
    }

    # averages per-subgroup
    subgroup_avg_score_cp = {
        g: {
            sg: _avg_from_sums_counts(
                subgroup_cp_sums[g][sg], subgroup_cp_counts[g][sg]
            )
    ...
    subgroup_avg_score_cr = {
        g: {
            sg: _avg_from_sums_counts(
                subgroup_cr_sums[g][sg], subgroup_cr_counts[g][sg]
            )
