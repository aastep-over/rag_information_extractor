...
def runtime_per_company(
    runtimes_data: Dict[str, Dict[str, Dict[str, str]]],
) -> Dict[str, Any]:
    per_group = {}
    overall = 0
    total_leafs = 0
    for group_name, group in runtimes_data.items():
        total_time_subgroups = sum(
            [unformat_time(sg["overall"]) for _, sg in group.items()]
        )
    ...
    return {"overall": format_time(overall / total_leafs), "per_group": per_group}
...
def runtime_overall(companies_runtimes: Dict[str, Any]) -> Dict[str, Any]:
    ...
    per_company = {
        name: runtime_per_company(md) for name, md in companies_runtimes.items()
    }
    ...
    for res in per_company.values():
        for g, t in res[
            "per_group"
        ].items():  # t is avg overall time per group g for company res
    ...
    }  # "per_company": per_company_formatted
