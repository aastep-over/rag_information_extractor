...
def find_covering_chunk_indices_difflib(
    ...
    for ref in refs:
        ...
        indices.append(
            best_idx if (best_idx is not None and best_cov >= threshold) else -1
        )

    return [chunks[i].get("id") for i in indices if i != -1]  # type: ignore


def format_context_ids_lists_in_json(
    context_ids: Dict[str, Dict[str, Dict[str, List[int]]]],
) -> str:
    """
...
def load_parent_chunks_from_dir(chunks_dir: str) -> List[Document]:
    ...
    parent_chunks = [
        ...
        for i in range(len(os.listdir(f"{chunks_dir}/page_content")))
    ]
    content_files = os.listdir(f"{chunks_dir}/page_content")
    metadata_files = os.listdir(f"{chunks_dir}/metadata")

    for i, (content_file, metadata_file) in enumerate(
        zip(content_files, metadata_files)
    ):
...
def load_children_chunks_from_chroma(chunks_dir: str, embedding) -> List[Document]:
    ...
    for i, (content, metadata) in enumerate(
        zip(vector_store.get()["documents"], vector_store.get()["metadatas"])
    ):
...
def find_azienda_chunk_ids(
    ...
    for group_name, group in raw_contexts.items():
        ...
        for sg_name, context in group_contexts.items():
            ...
            rel_chunks_children = [
                {"text": d.page_content, "id": d.metadata.get("chunk_id")}
                for d in children_chunks
                if d.metadata.get("azienda") == azienda_name.lower()
            ]
            rel_ids_children = find_covering_chunk_indices_difflib(
                [context], rel_chunks_children
            )
            ...
            rel_ids_parents = find_covering_chunk_indices_difflib(
                [context], rel_chunks_parents
            )
...
def build_combined_chunk_json(
    ...
    # Save updated json
    combined_raw_json_path.write_text(
        json.dumps(combined_raw_data, indent=4, ensure_ascii=False), encoding="utf-8"
    )
...
def analyze_raw_contexts_coverage(
    ...
    def _read_parent_chunk(chunk_id: int) -> str:
        try:
            with open(
                f"{parent_chunks_dir}/page_content/{chunk_id}", encoding="utf-8"
            ) as f:
                return f.read()
        except Exception:
            return ""
    ...
    for azienda, data in combined_raw_json.items():
        for group_key, group in data.items():
            ...
            for (
                ...
                raw_contexts_per_group.keys()
            ):  # raw_contexts and raw_contexts_ids have same subkeys (subgroup names)
                raw_context = raw_contexts_per_group[sub_key]  # context (str)
                raw_context_ids = raw_contexts_ids_per_group[
                    sub_key
                ]  # {"parent": [], "children: []"}
                ...
                parent_chunks = [
                    _read_parent_chunk(pid) for pid in parent_ids if pid is not None
                ]  # _normalize(_read_parent_chunk(pid))
                child_chunks = [
                    _read_child_chunk(cid) for cid in child_ids if cid is not None
                ]  # _normalize(_read_child_chunk(cid))

                # Check coverage of ref_context in each parent and child chunk
                parent_coverage = _context_coverage(
                    ref_context, " ".join(parent_chunks)
                )
