...
def load_company_dicts(
    combined_raw_json: Path,
    pred_json: Path,
    match_scores_json: Path,
    match_scores_qa_json: Path,
) -> Tuple[
    ...
    Dict[
        str, Any
    ],  # companies_pred_contexts (we save only start and end part not whole context)
    ...
    if not match_scores_json.exists():
        raise FileNotFoundError(
            f"Match scores json file not found: {match_scores_json}"
        )
    ...
    try:
        with combined_raw_json.open("r", encoding="utf-8") as f:
            raw_data = json.load(
                f
            )  # json tree: azienda -> info_schema -> (values, raw_contexts, raw_contexts_ids, raw_qa)
    except Exception as e:
        print(
            f"Skipping {combined_raw_json} — failed to read/parse JSON: {e}",
            file=sys.stderr,
        )
    ...
    for azienda, group in raw_data.items():
        ...
        for group_name, group_data in group.items():
            ...
            companies_raw_contexts_ids[azienda][group_name] = group_data.get(
                "raw_contexts_ids"
            )
    ...
    try:
        with pred_json.open("r", encoding="utf-8") as f:
            results = json.load(
                f
            )  # json tree: azienda -> info_schema -> (output, retrieved_docs, re_ranked_docs, rag_qa, run_times, retrieved_docs_texts, re_ranked_docs_texts, optimized_query)
    except Exception as e:
    ...
    try:
        ...
        print(
            f"Skipping {match_scores_json} — failed to read/parse JSON: {e}",
            file=sys.stderr,
        )
        companies_match_data = {}
    ...
    try:
        ...
        print(
            f"Skipping {match_scores_qa_json} — failed to read/parse JSON: {e}",
            file=sys.stderr,
        )
        companies_match_qa = {}
...
if __name__ == "__main__":
    ...
    match_scores_json = (
        BASE_DIR / "runs/TRAIN/custom_chunks/run_2026-03-09 09-11-36/match_scores.json"
    )
    match_scores_qa_json = (
        BASE_DIR
        / "runs/TRAIN/custom_chunks/run_2026-03-09 09-11-36/match_scores_qa.json"
    )
