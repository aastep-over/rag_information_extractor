...
def semantic_chunking(
    ...
    parent_chunks = [
        ...
        if (pc.page_content not in pages_joining_str if pages_joining_str else True)
        and (pc.page_content != "")
    ]
...
async def semantic_chunking_async(
    ...
    async def _chunk_one(d: Document) -> List[Document]:
        ...
        async with sem:
            ...
            return await asyncio.to_thread(
                semantic_chunker.create_documents, [d.page_content], [d.metadata]
            )

    per_doc_chunks = await asyncio.gather(*[_chunk_one(d) for d in docs])
    parent_chunks: List[Document] = [
        pc
        for sub in per_doc_chunks
        for pc in sub
        if (pc.page_content not in pages_joining_str if pages_joining_str else True)
        and (pc.page_content != "")
    ]
        ...
        async with sem:
            ...
            if p_tokens > max_embed_tokens:
                subs: List[Document] = await asyncio.to_thread(
                    text_splitter.split_documents, [p]
                )
            else:
    ...
    children_lists = await asyncio.gather(
        *[_split_children_from_parent(p) for p in parent_chunks]
    )
