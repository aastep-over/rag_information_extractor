...
class Rimborso(BaseModel):
    ...
    """
    question: ClassVar[str] = "Agli amministratori spetta il rimborso delle spese? Quali spese sono incluse?"
...
class IndennitaAnnuale(BaseModel):
    ...
    """
    question: ClassVar[str] = "I soci possono assegnare un compenso agli amministratori? In che misura?"
    ...
    misura_indennita: str = Field(
        default="",
        description=(
            'Solo dalla RISPOSTA: misura di compensione; '
            'se non presente → "".'
        )
    )
...
class IndennitaCessazione(BaseModel):
    ...
    """
    question: ClassVar[str] = "I soci possono accantonare una somma a titolo di indennità per gli amministratori in caso di cessazione della carica?"
