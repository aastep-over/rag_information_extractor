...
class OllamaLLM:
    ...
    def get_response_text(
        ...
        try:
            ...
            raise ConnectionError(f"Errore di connessione a Ollama ({host}/{model_name}): {e}")
    ...
    def invoke(
        self,
        output_format: Literal["text", "structured", "json"] = "text",
        **kwargs
        ...
        if output_format == "structured":
            ...
            if not schema:
                raise Exception("Missing info schema (kwarg: 'info_schema')! Provide the BaseModel schema for the information to be extracted!")
            try:
                completion = self.get_structured_response(
                    prompt_content=prompt_content, info_schema=schema, temperature=temperature,
                    model_name=model_name, host=host, cache=use_cache, num_predict=num_predict
                )
            ...
            try:
                completion = self.get_response_text(
                    prompt_content=prompt_content, temperature=temperature, model_name=model_name,
                    host=host, cache=use_cache, format="json", num_predict=num_predict
                )
            except ConnectionError as e:
                completion = str(e) + ". Assicurati che Ollama sia in esecuzione."
        else:
            try:
                completion = self.get_response_text(
                    prompt_content=prompt_content, temperature=temperature, model_name=model_name,
                    host=host, cache=use_cache, num_predict=num_predict
                )
            except ConnectionError as e:
                completion = str(e) + ". Assicurati che Ollama sia in esecuzione."
    ...
    async def aget_response_text(
        ...
        try:
            ...
            raise ConnectionError(f"(async) Errore di connessione Ollama ({host}/{model_name}): {e}")
        ...
        if output_format == "structured":
            ...
            try:
                return await self.aget_structured_response(
                    prompt_content, schema, model_name, host,
                    cache=use_cache, num_predict=num_predict
                )
            except ConnectionError as e:
            ...
            try:
                completion = await self.aget_response_text(
                    prompt_content, model_name, host, format="json",
                    cache=use_cache, num_predict=num_predict
                )
            except ConnectionError as e:
                completion = str(e) + ". Assicurati che Ollama sia in esecuzione."
            ...
            try:
                completion = await self.aget_response_text(
                    prompt_content, model_name, host,
                    cache=use_cache, num_predict=num_predict
                )
            except ConnectionError as e:
                completion = str(e) + ". Assicurati che Ollama sia in esecuzione."
...
if __name__ == "__main__":
    ...
    async def aextract_all(infos_to_extract):
        ...
        results = [await llm.ainvoke(memory=info.get("prompt"), output_format="structured", info_schema=info.get("schema")) for info in infos_to_extract_processed]
        for result in results:
            print(result.model_dump()) # type: ignore
