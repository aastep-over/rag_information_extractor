...
# from other modules
from rag_info_extractor.utils.apis_connector import call_reranker_service, acall_reranker_service
...
async def across_encode_rerank(
    ...
    query = question
    context_candidates = contexts 
    scores = await acall_reranker_service(
        query = question,
        documents = [d.page_content for d in context_candidates]
    ) or []
...
async def afaster_retrieve_and_rerank(
    ...
    docs = [docs[i] for i in idx_to_include] # TODO: If len(idx_to_include) very large, then implement it inside asyncio.to_thread

    # remove page joining str to avoid confusion for llm in the generation phase
    if pages_joining_str:
        for d in docs: # TODO: If len(docs) very large, then implement it inside asyncio.to_thread
            d.page_content = d.page_content.replace(pages_joining_str, "\n")
