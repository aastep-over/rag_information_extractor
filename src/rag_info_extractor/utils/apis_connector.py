...
async def acall_reranker_service(
...
def call_pruner_service(query: str, documents: List[str]) -> List[str]:
    ...
    try:
        response = requests.post(
            PRUNER_API, json=payload, headers={"Content-Type": "application/json"}
        )
        response.raise_for_status()  # Launches an exception for error status
        result = response.json()
...
def call_embedder_service(texts: List[str], **encode_kwargs) -> List[List[float]]:
    ...
    payload = {"texts": texts, "encode_kwargs": encode_kwargs}

    try:
        response = requests.post(
            EMBEDDING_API, json=payload, headers={"Content-Type": "application/json"}
        )
        response.raise_for_status()  # Launches an exception for error status
...
async def acall_embedder_service(
    texts: List[str], **encode_kwargs
) -> List[List[float]]:
    """Chiama il servizio di Embedder esterno."""
    payload = {"texts": texts, "encode_kwargs": encode_kwargs}
