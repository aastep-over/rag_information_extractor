...
def main(functions_to_run: list[str]):
    ...
    with open("output_temp", "w", encoding="utf-8") as f:
        f.write(
            f"## OUTPUT FOR: re_ranker.py \n{datetime.datetime.strftime(datetime.datetime.now(), '%Y-%m-%d %H:%M:%S')}\n\n"
        )

        # Save outputs of cross_encode_reranker
        if ("cross_encode_rerank" in functions_to_run) or (
            "across_encode_rerank" in functions_to_run
        ):
            f.write(
                f"# FUNCTION: {'cross_encode_rerank' if 'cross_encode_rerank' in functions_to_run else '(async) cross_encode_rerank'}\n"
            )
            ...
            f.write(
                f'children =  {output.get("re_ranked_docs_ids").get("children")}\n\n\n'
            )
            ...
            f.write(
                f'children =  {output.get("re_ranked_docs_texts").get("children")}\n'
            )
        ...
        if ("faster_retrieve_and_rerank" in functions_to_run) or (
            "afaster_retrieve_and_rerank" in functions_to_run
        ):
            f.write(f'\n{"x"*100}\n')
            f.write(f'{"x"*100}\n\n')

            f.write(
                f"# FUNCTION: {'faster_retrieve_and_rerank' if 'faster_retrieve_and_rerank' in functions_to_run else '(async) faster_retrieve_and_rerank'}\n"
            )
            ...
            f.write(f'{"x"*100}\n')
            f.write(
                f'Re-Ranked Doc Texts: {fast_reranker_output.get("docs_texts")}\n\n'
            )
...
if __name__ == "__main__":
    ...
    functions_to_run = [
        "across_encode_rerank"
    ]  # in ["cross_encode_rerank", "faster_retrieve_and_rerank"]  # can be both (and their async versions)
